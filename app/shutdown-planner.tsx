...
export default function ShutdownPlanner() {
  ...
  const [textDraft, setTextDraft] = useState<{ key: string; label: string; content: string; hours?: number } | null>(null);
  function openTextEditor(key: string, row: ActivityRow, label: string, hour?: number) {
    if (!canEditRow(row)) return;
    const selected = selectedTextCellRef.current;
    const target = hour ?? (selected?.key === key ? selected.hour : 0);
    const merge = mergesFor(key).find(m => target >= m.start && target <= m.end);
    const first = merge?.start ?? target;
    const textKey = `${key}-h-${first}`;
    setTextDraft({ key: textKey, label: `${label} · cột ${first+1}`, content: notes[textKey] || "", hours: row.kind.startsWith("detail") ? (merge ? merge.end-merge.start+1 : 1) : undefined });
  }
  function saveTextDraft() {
    if (!textDraft || !canWriteNote(editRole,textDraft.key)) return;
    commitEdit({ ...cells }, { ...notes, [textDraft.key]: textDraft.content });
    setTextDraft(null);
  }
  ...
  useEffect(() => { undoStack.current = []; redoStack.current = []; selectRange(null); setPasteDraft(null); setLateView(null); setLateDraft(null); setTextDraft(null); }, [editRole]);
  ...
  function renderTextTimeline(process: Process, row: ActivityRow, key: string) {
    ...
    while (hour < HOURS.length) {
      ...
      output.push(<div key={textKey} data-grid-row={rowIndex} data-grid-hour={cellHour} onPointerDown={selectCell} style={{ gridColumn: `span ${span}`, backgroundColor: selectedColor }} className={`timeline-text ${span > 1 ? "merged-text" : ""} ${inRange(rowIndex,cellHour,span) ? "selected-text-cell" : ""} ${(timelineStartHour - 1 + cellHour) % 24 === 0 ? "day-start" : ""}`}><div className="detail-content" onDoubleClick={() => openTextEditor(key,row,`${process.name} · ${row.label}`,cellHour)} title={editable ? "Nhấp đúp để sửa nội dung, hoặc dùng nút Sửa chữ ở đầu dòng" : undefined}><span className="timeline-text-editor">{notes[textKey] || (cellHour === 0 ? (row.kind === "note" ? "Ghi chú…" : "Chi tiết…") : "")}</span>{row.kind.startsWith("detail") && (span > 1 || Boolean(notes[textKey])) && <span className="activity-hours">({span})</span>}</div></div>);
      hour += span;
    }
    return output;
  }
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#e0f2fe_0,transparent_30%),linear-gradient(145deg,#f8fbff,#effcf7)] text-slate-900">
    ...
    <Dialog open={Boolean(textDraft)} onOpenChange={open => { if (!open) setTextDraft(null); }}><DialogContent className="max-w-md"><DialogHeader><DialogTitle>Sửa nội dung timeline</DialogTitle><DialogDescription>{textDraft?.label}{textDraft?.hours !== undefined ? ` · ${textDraft.hours} giờ (tự động)` : ""}</DialogDescription></DialogHeader><label className="late-reason-label">Nội dung<textarea autoFocus aria-label="Nội dung timeline" rows={4} maxLength={300} value={textDraft?.content || ""} onChange={e => setTextDraft(current => current ? { ...current, content: e.target.value.slice(0,300) } : null)}/></label><small>Số giờ tự hiển thị sau nội dung. Bấm Cập nhật trên trang để lưu lên máy chủ.</small><DialogFooter><Button variant="outline" onClick={() => setTextDraft(null)}>Hủy</Button><Button onClick={saveTextDraft}>Áp dụng</Button></DialogFooter></DialogContent></Dialog>
